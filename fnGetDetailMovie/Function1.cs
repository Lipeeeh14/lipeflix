...
public class Function1
{
    ...
    public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Function, "get")] HttpRequestData req)
    {
        ...
        var container = _cosmosClient.GetContainer("LipeFlixDB", "movies");
        var id = req.Query["id"];      
        var query = new QueryDefinition($"SELECT * FROM c WHERE c.id = @id");
        var queryDefinition = query.WithParameter("@id", id);
        ...
        {
            foreach (var item in result.ReadNextAsync().Result)
            {
                results.Add(item);
            }
        }
